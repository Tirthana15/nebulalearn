...
function calculateSubmissionSpeed($studentId) {
    ...
    $query = "SELECT AVG(TIMESTAMPDIFF(HOUR, a.created_at, sa.submitted_at)) as avg_speed 
              FROM student_assignments sa
              JOIN assignments a ON sa.assignment_id = a.id
              WHERE sa.student_id = ? AND sa.submitted_at IS NOT NULL";
    ...
    if ($result->num_rows > 0) {
        ...
        return $row['avg_speed'] ? round($row['avg_speed'], 2) : 0;
    }
    
    return 0;
}

function formatSubmissionSpeed($hours) {
    if ($hours < 1) {
        $minutes = round($hours * 60);
        return $minutes > 0 ? $minutes . ' mins' : 'N/A';
    } elseif ($hours < 24) {
